...
class MultiheadAttention(torch.nn.MultiheadAttention, KQI):
    ...
    def KQIbackward(self,
        ...
        logging.debug(f'volume_7_all: KQI={KQI.kqi}, node={np.prod(volume_7_all.shape)}, volume={volume_7_all.sum()}')
        ...
        for col in volume_2_v:
            for vol in col:
                KQI.kqi += self.KQI_formula(volume_7[:, 0] / (seq_len * 2), vol) * num_heads
        logging.debug(f'volume_6: KQI={KQI.kqi}, node={np.prod(volume_6.shape)}, volume={volume_6.sum()}')
        ...
        logging.debug(f'volume_5: KQI={KQI.kqi}, node={np.prod(volume_4.shape)}, volume={volume_4.sum()}')
        ...
        logging.debug(f'volume_3: KQI={KQI.kqi}, node={np.prod(volume_3.shape)}, volume={volume_3.sum()}')
        ...
        for col in volume_2_k:
            for vol in col:
                KQI.kqi += self.KQI_formula(volume_3[0, :] / (head_dim * 2), vol) * num_heads
        logging.debug(f'volume_2_q: KQI={KQI.kqi}, node={np.prod(volume_2_q.shape)}, volume={volume_2_q.sum()}')
        logging.debug(f'volume_2_k: KQI={KQI.kqi}, node={np.prod(volume_2_k.shape)}, volume={volume_2_k.sum()}')
        logging.debug(f'volume_2_v: KQI={KQI.kqi}, node={np.prod(volume_2_v.shape)}, volume={volume_2_v.sum()}')
        ...
        logging.debug(f'volume_1_q: KQI={KQI.kqi}, node={np.prod(volume_1_q.shape)}, volume={volume_1_q.sum()}')
        logging.debug(f'volume_1_k: KQI={KQI.kqi}, node={np.prod(volume_1_k.shape)}, volume={volume_1_k.sum()}')
        logging.debug(f'volume_1_v: KQI={KQI.kqi}, node={np.prod(volume_1_v.shape)}, volume={volume_1_v.sum()}')
