...
def test_Conv3d():
    class TestConv3d(torch.nn.Module, kqinn.KQI):
        def __init__(self) -> None:
            ...
            self.layers2 = kqinn.Sequential(
                # 3x8x8x8
                kqinn.Linear(in_features = 3*8*8*8, out_features = 100, bias=False),
                kqinn.Linear(in_features = 100, out_features = 10, bias=False),
            )
        ...
        def true_kqi(self):
            ...
            for i,j,k in itertools.product(range(28), range(28), range(28)):
                preds = [f'L1_{m1}-{m2}-{m3}_{m4}' for m1,m2,m3 in itertools.product([i-1,i,i+1], [j-1,j,j+1], [k-1,k,k+1]) if m1>=0 and m1<28 and m2>=0 and m2<28 and m3>=0 and m3<28 for m4 in [1,2,3]]
            ...
            for i in range(100):
                preds = [f'L3_{m1}-{m2}-{m3}_{m4}' for m1,m2,m3 in itertools.product(range(8), range(8), range(8)) for m4 in [1,2,3]]
                G.add_node(f'L4_{i}', preds)
    ...
    kqi = TestConv3d().KQI(torch.randn(3,28,28,28))
