...
def test_TransformerDecoderLayer():
    ...
    class TestTransformerDecoderLayer(torch.nn.Module, kqinn.KQI):
        ...
        def KQIforward(self, x: torch.Tensor) -> torch.Tensor:
            x = x.flatten()
            tgt_mem = self.layer.KQIforward(x).reshape(sequence_length, d_model * 2)
            tgt = tgt_mem[:, :d_model].reshape(sequence_length, d_model)
            mem = tgt_mem[:, d_model:].reshape(sequence_length, d_model)
        ...
        def KQIbackward(self, volume: torch.Tensor, volume_backward_x: torch.Tensor = None, volume_backward_memory: torch.Tensor = None) -> torch.Tensor:
            return self.layer.KQIbackward(volume, volume_backward_x)
    ...
    tgt = torch.randn(batch_size, sequence_length, d_model)
    memory = torch.rand(batch_size, sequence_length, d_model)
    decoder_layer = TestTransformerDecoderLayer()
    decoder_layer.set_memory(memory)
    kqi = decoder_layer.KQI(tgt)
    true = decoder_layer.true_kqi()
